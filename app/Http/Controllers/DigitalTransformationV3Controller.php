...
class DigitalTransformationV3Controller extends Controller
{
    function index(Request $request) {
        if(isset($request['qr'])){
            if (!$request->hasCookie('tracked')) {
                $record = Access::where('origen', 'QR-V3')->first();
                ...
                return response()->view('transformacion-digital-v3')->cookie($alreadyTracked);
            }
        }
        return view('transformacion-digital-v3');
    }
